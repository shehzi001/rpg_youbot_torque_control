...
TorqueController::TorqueController(ros::NodeHandle& nh, std::string name) :
    traj(nh, name, boost::bind(&TorqueController::followTrajectory, this, _1), false), DOF(5), duration(0),traj_follow(nh, name, boost::bind(&TorqueController::generateSparseTrajectory, this, _1),false)
...
void TorqueController::generateSparseTrajectory(const control_msgs::FollowJointTrajectoryGoalConstPtr & trajectory)
{
  ...
  //cout << "Trajectory generated by the trajectory generator:" << j_traj;
   
  joint_points.resize(j_traj.points.size());
  joint_vels.resize(j_traj.points.size());
    
  int noofPoints = 0;
  int size = j_traj.points.size(); 
  while (!j_traj.points.empty())
  { 
    joint_points[noofPoints].positions.resize(5);
    joint_vels[noofPoints].velocities.resize(5);
    
    for(int j = 0;j < 5;j++)
    {
     joint_points[noofPoints].positions[j].joint_uri = j_traj.joint_names[j];
     joint_points[noofPoints].positions[j].unit = "rad";
     joint_points[noofPoints].positions[j].value = j_traj.points[noofPoints].positions[j];
     
     joint_vels[noofPoints].velocities[j].joint_uri = j_traj.joint_names[j];
     joint_vels[noofPoints].velocities[j].unit = "s^-1 rad";
     joint_vels[noofPoints].velocities[j].value = max_vel;// j_traj.points[noofPoints].velocities[j];
  ...
  int index = 0;
  while (index < (noofPoints-1)) 
   {
    js2js.request.start_pos = joint_points[index];
    js2js.request.start_vel = joint_vels[index];
    index++;
    js2js.request.end_pos = joint_points[index];
    js2js.request.end_vel = joint_vels[index];
    js2js.request.max_vel = max_vel;
    js2js.request.max_acc = max_acc;
   
   //cout << "Requesting js2js service" << endl;
    if (js2js_client.call(js2js))
    {
      //cout << "call successful" << endl;
      if (js2js.response.feasible)
      {
        cout << "Tranjectory feasible" << endl ;
        feasible = true;
      ...
      {
        cout << "Tranjectory Not Feasible" << endl;
        feasible = false;
        //break;
      }
     }
   ...
   if(feasible == true)
   {
      //followTrajectory_moveit(traj);
      traj_follow.setSucceeded(result_controlmsgs);
   }
   else 
   {
     cout << "Trajectory can not be executed:";
      traj_follow.setAborted(result_controlmsgs);
   }
}

void TorqueController::followTrajectory_moveit(trajectory_msgs::JointTrajectory j_traj)
{
  mode = "trajectory";
