...
TorqueController::TorqueController(ros::NodeHandle& nh, std::string name) :
    traj(nh, name, boost::bind(&TorqueController::followTrajectory, this, _1), false), DOF(5), duration(0),traj_follow(nh, name, boost::bind(&TorqueController::generateDenseTrajectory, this, _1),false)
...
void TorqueController::generateDenseTrajectory(const control_msgs::FollowJointTrajectoryGoalConstPtr &trajectory)
{
  ...
  while (iterator < (trajectory_in.points.size() - 1)) 
   ...
   {     
     ...
     followDenseTrajectory(trajectory_out);
   }
   else 
   {
...
void TorqueController::followDenseTrajectory(trajectory_msgs::JointTrajectory trajectory)
{
  mode = "trajectory";
  ...
  while (!trajectory.points.empty())
  {
    ...
    ros::spinOnce();
    if(!pointToEigen(q_tra, qdot_tra, qdotdot_tra, trajectory.points.back(),trajectory.joint_names))
    {
      ros::shutdown();
    }
...
void TorqueController::followTrajectoryControlMsgs(const control_msgs::FollowJointTrajectoryGoalConstPtr & trajectory)
{
  mode = "trajectory";
  ROS_INFO("Following Trajectory");
  Eigen::VectorXd pos_err(5), vel_err(5), a_pos(5), d_pos(5);
  ...
  trajectory_msgs::JointTrajectory trajectory_in = trajectory->trajectory;
   ...
   cout << "Trajectory generated by the trajectory generator:" << trajectory_in;
   
  
  while (!trajectory_in.points.empty())
  { 
    ros::spinOnce();
    if(!pointToEigen(q_tra, qdot_tra, qdotdot_tra, trajectory_in.points.back(),trajectory_in.joint_names))
    {
      ros::shutdown();
    }
    trajectory_in.points.pop_back();
...
void TorqueController::followTrajectory(const torque_control::torque_trajectoryGoalConstPtr & trajectory)
{
  ...
  //cout << "Trajectory generated by the trajectory generator:" << j_traj << endl;
  ...
  while (!j_traj.points.empty())
  {
    ...
    torque_command_pub.publish(generate_joint_torque_msg(m_torques));
    cout << "joint torque msgs" << endl << generate_joint_torque_msg(m_torques);
    ros::spinOnce();
    loop_rate.sleep();
  }
  pos_command_pub.publish(stop);
